...
def get_ray_directions_cam(uv, intrinsics, orthogonal=False):
    ...
    pixel_points_cam = lift(
        x_cam, y_cam, z_cam, intrinsics=intrinsics, 
...
def lift(x, y, z, intrinsics, homogeneous=False, orthogonal=False):
    ...
    if orthogonal:
        # x_lift = (expand_as(cx, x) - x) / expand_as(fx, x)
...
def world_from_xy_depth(xy, depth, cam2world, intrinsics, orthogonal=False):
    ...
    pixel_points_cam = lift(
        x_cam, y_cam, z_cam, intrinsics=intrinsics, 
        homogeneous=True, orthogonal=orthogonal)  # (batch_size, num_samples, 4)
...
def get_ray_directions(xy, cam2world, intrinsics, orthogonal=False):
    ...
    if orthogonal:
        ...
        pixel_points = world_from_xy_depth(
            ...
            cam2world=cam2world, 
            orthogonal=orthogonal)  # (batch, num_samples, 3)
