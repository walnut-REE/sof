...
class Linear(nn.Module):
    def __init__(self, in_dim, out_dim, z_dim, hidden_ch=256, hidden_layer=6):
        super().__init__()

        self.linear, self.bias = nn.ModuleList(), nn.ModuleList()
        node_num = [in_dim] + [hidden_ch]*hidden_layer + [out_dim]
        self.layer_num = len(node_num) - 1
        for i in range(self.layer_num):
            self.linear.append(nn.Linear(node_num[i], node_num[i+1], bias=False))
            self.bias.append(nn.Linear(z_dim, node_num[i+1], bias=True))

    def forward(self, x, z):
        if z.dim()<3:
            B,N,C = x.shape
            z_in = z[:,None].expand(-1,N,-1)
        else:
            z_in = z
        for i in range(self.layer_num):
            x = self.linear[i](x) + self.bias[i](z_in)
