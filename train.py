...
from modeling.srns import SRNsModel
import util
import random
import re
import shutil
...
def train():
    ...
    now = datetime.now()
    task_name = opt.name if opt.name else os.path.splitext(os.path.basename(opt.config_filepath))[0]
    ...
    while True:
        for model_input, ground_truth in train_dataloader:
            ...
            model_outputs = model(pose, z, intrinsics, uv, dpt_scale=opt.dpt_scale)
